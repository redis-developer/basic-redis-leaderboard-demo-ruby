...
export default {
  ...
  data: () => ({
    ...
    ACTION_LIST: [
      ...
      { text: 'Between Rank 10 & 15', value: 'between_rank&&start=9&end=14' },
      { text: 'Show Ranks of AAPL, FB and TSLA', value: 'symbols&&values=[aapl, fb, tsla]' },
  ...
  methods: {
    async onUpdateRank () {
      ...
      try {
        ...
        await axios.get(`${API_BASE}/companies/update`, { params: form })
    ...
    async loadData () {
      ...
      try {
        ...
        const apiResp = await axios.get(`${API_BASE}/companies?sort=${method}`)
